...
/* Neon Royale Roulette v4
   The current CHIP amount applies only to the NEXT click.
   Every betting spot remembers the dollars already placed on it.
*/
(function rouletteChipAmountsV4(){
  ...
  window.renderRoulette=function renderRouletteChipAmounts(){
    let spinning=false,rotation=0,lastTick=-1;
    const bets=new Map(); // key -> total dollars placed on this spot
    ...
    $('#gameMount').innerHTML=`<div class="casino-room room-roulette roulette-integrity roulette-multibet-v3 roulette-chip-v4">
      <div class="room-topline"><span class="rules-badge">AMERICAN WHEEL · SET CHIP VALUE · CLICK ANY SPOT</span>${betControls('rouletteStake',100)}</div>
      <div class="roulette-stage">
        <div class="roulette-machine ri-machine">
          ...
          <small class="ri-integrity-note">The amount in CHIP is placed only when you click a betting spot. Existing chips keep their original value.</small>
        </div>
        <div class="betting-board">
          <p class="kicker">SET CHIP VALUE, THEN CLICK</p>
          <div class="roulette-multi-summary" id="rouletteMultiSummary"><b>$0 ON TABLE</b><small>Example: set $1,000 and click 14, then set $100 and click 17.</small></div>
    ...
    const totalOnTable=()=>[...bets.values()].reduce((a,b)=>a+b,0);

    function paint(){
      $$('[data-rbet]').forEach(b=>{
        const amount=bets.get(b.dataset.rbet)||0;
        b.classList.toggle('active',amount>0);
        const badge=$('.roulette-chip-count',b);if(badge)badge.textContent=amount?money(amount):'';
      });
      const next=Number($('#rouletteStake')?.value)||0,total=totalOnTable();
      summary.innerHTML=`<b>${money(total)} ON TABLE</b><small>Next click: ${money(next)} · change CHIP anytime before placing another bet</small>`;
    }

    $$('[data-rbet]').forEach(b=>b.onclick=()=>{
      if(spinning)return;
      const chip=Number($('#rouletteStake')?.value);
      if(!Number.isFinite(chip)||chip<=0)return toast('Set a chip amount first');
      const k=b.dataset.rbet;bets.set(k,(bets.get(k)||0)+chip);pulse(b,'chip-drop');paint();
    });
    ...
    $('#spinRoulette').onclick=async()=>{
      if(spinning)return;
      const totalStake=totalOnTable();if(totalStake<=0)return toast('Place at least one roulette chip');
      ...
      for(const [key,wager] of bets){const p=payout(key,result,wager);if(p.won){totalReturn+=p.ret;winners.push(`${labelOf(key)} ${money(wager)}`)}}
